tabby_calib_graphs <- function(loc, out_i) {
  ...
  p <- recordPlot()
  saveRDS(p, file=paste0('utilities/inst/calibration_plots/',loc,'/total_population.rds'))
  ...
  #'format the plot
  plot(1,1,ylim=c(2,100),xlim=c(2006,2016),xlab="",ylab="",axes=F, log = "y")
  axis(1);axis(2,las=2);box()
  abline(h=axTicks(2),col="grey85")
  ...
  points(2006:2016,notif_fb[14:24,2],pch=19,cex=0.3,col=4) #US born population
  lines(2006:2016,notif_fb[14:24,2],pch=19,lty=3,col=4)
  ...
  #'plot text
  mtext("Year",1,2.5,cex=0.9)
  mtext("Total TB Cases Identified (000s), 2006-2016",3,.8,font=2,cex=0.8)
